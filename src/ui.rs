...
fn render_man_viewer(f: &mut Frame, app: &App, area: Rect) {
    ...
    let lines: Vec<Line> = app
        ...
        .take(viewer_height)
        .map(|(line_idx, line)| {
            if !app.in_page_search_query.is_empty() && app.in_page_search_matches.contains(&line_idx) {
...
fn render_footer(f: &mut Frame, app: &App, area: Rect) {
    ...
    let help_text = if app.focus == FocusState::InPageSearch {
        ...
        "[↑↓] Navigate  [Enter] Open  [Tab] Switch  [/] Search  [s] Section  [f] Favorite  [t] Theme  [q] Quit"
    };
